...
For architectural description, see [README-azure.md](./README-azure.md) originally from [PR#35](https://github.com/tiiuae/ghaf-infra-ssd/pull/35)
...
- The `terraform/persistent` directory contains the terraform configuration for parts of the infrastructure that are considered persistent - resources defined under `terraform/persistent` will not be removed even if the ghaf-infra-ssd instance is otherwise removed. An example of such persistent ghaf-infra-ssd resource is the binary cache storage as well as the binary cache signing key. There may be many 'persistent' infrastructure instances - currently `priv` `dev/prod` and `release` deployments have their own instances of the persistent resources. Section [Multiple Environments with Terraform Workspaces](./README.md#multiple-environments-with-terraform-workspaces) discusses this topic with more details.
- The `terraform/state-storage` directory contains the terraform configuration for the ghaf-infra-ssd remote backend state storage using Azure storage blob. See section [Initializing Azure State and Persistent Data](./README.md#initializing-azure-state-and-persistent-data) for more details.
- The `terraform/modules` directory contains terraform modules used from the ghaf-infra-ssd VM configurations to build, upload, and spin up Azure nix images.
...
- Different workspaces allow deploying different instances of ghaf-infra-ssd. Each instance has a completely separate state data, making it possible to deploy `dev`, `prod`, `release` or even private development instances of ghaf-infra-ssd. This makes it possible to first develop and test infrastructure changes in a private development environment, before proposing changes to shared (e.g. `dev` or `prod`) environments. The configuration codebase is the same between all the environments, with the differentiation options defined in the [`main.tf`](./main.tf#L105).
- Parts of the ghaf-infra-ssd infrastructure are persistent and shared between different environments. As an example, private environments share the binary cache storage. This arrangement makes it possible to treat, for instance, private ghaf-infra-ssd instances dispensable: ghaf-infra-ssd instances can be temporary and short-lived as it's easy to spin-up new environments without losing any valuable data. The persistent data is configured outside the root ghaf-infra-ssd terraform deployment in the `terraform/persistent` directory. There may be many 'persistent' infrastructure instances - currently `priv`, `dev/prod` and `release` deployments have their own instances of the persistent resources. This means that `priv`, `dev/prod` and `release` instances of ghaf-infra-ssd do **not** share any persistent data. As an example, `priv` and `prod` deployments of ghaf-infra-ssd have a separate binary cache storage. The binding to persistent resources from ghaf-infra-ssd is done in the [`main.tf`](./main.tf) based on the terraform workspace name and resource location. Persistent data initialization is automatically done with `terraform-init.sh` script.

To help facilitate the usage of terraform workspaces in setting-up distinct copies of ghaf-infra-ssd, one can [use terraform workspaces from the command line](https://developer.hashicorp.com/terraform/cli/workspaces#managing-cli-workspaces). Below, for the sake of example, we setup a private deployment instance of ghaf-infra-ssd:

```bash
# Activate private development environment 'henri'
❯ ./terraform-init.sh -w henri
[+] Using state 'ghaf-infra-ssd-0-state-eun'
[+] Using persistent 'ghaf-infra-ssd-0-persistent-eun'
...
# Make sure you deploy to the correct ghaf-infra-ssd instance.
...
```bash
❯ ./terraform-init.sh -w workspacename
[+] Using state 'ghaf-infra-ssd-state-0-eun'
[+] Using persistent 'ghaf-infra-ssd-persistent-0-eun'
[+] Initializing workspace-specific persistent
╷
│ Error: Backend configuration changed
│
...
│ Resource Group Name: "ghaf-infra-ssd-henri"
│ Image Name: "binary-cache"): performing CreateOrUpdate: unexpected status 400 (400 Bad Request) with error: InvalidParameter: The source blob https://imghenrieun.blob.core.windows.net/ghaf-infra-ssd-vm-images/binary-cache.vhd is not accessible.
│
